...
class Model_Training(Data_Sourcing):
    '''
    ...
    def split_data(self,data,test_size=0.2,stratify=True,random_state=42):
        '''
        ...
        if stratify:
            X_train,X_test,y_train,y_test=train_test_split(data.drop(['EPP_TAKER'],axis=1),data['EPP_TAKER'],test_size=test_size,stratify=data['EPP_TAKER'],random_state=random_state)
        else:
            X_train,X_test,y_train,y_test=train_test_split(data.drop(['EPP_TAKER'],axis=1),data['EPP_TAKER'],test_size=test_size,random_state=random_state)
    ...
    def train_model(self, X_train, X_test, y_train, y_test):
        ...
        combine them into a weighted Voting Ensemble, 
        and save the final ensemble model.
        """
        ...
        # Create weighted voting ensemble (soft voting)
        voting_clf = VotingClassifier(
            estimators=[(n, m) for n, m in models.items()],
            voting="soft",
            weights=[model_weights["RandomForest"], model_weights["XGBoost"], model_weights["LightGBM"]]
        )
        ...
        print(f"Ensemble (Weighted Voting) Accuracy: {acc_vote:.4f}")

        # Save the final ensemble model
        joblib.dump(voting_clf, os.path.join(os.path.dirname(os.path.abspath(__file__)), "ensemble_model.pkl"))
